...
export default class Draggable extends React.Component<DraggableProps, DraggableState> {
  ...
  static propTypes = {
     ...
     * 'exclude' to allow either horizontal or vertical but not both
     *
     * Defaults to 'both'.
     */
    axis: PropTypes.oneOf(['both', 'x', 'y', 'none', 'exclude']),
  ...
  onDrag: DraggableEventHandler = (e, coreData) => {
    ...
    if (this.props.axis === 'exclude' && this.state.currentAxis === 'none') {
      this.setState({
        currentAxis: Math.abs(uiData.deltaX) > Math.abs(uiData.deltaY) ? 'x' : 'y',
      });
    }
